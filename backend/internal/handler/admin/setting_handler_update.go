...
func (h *SettingHandler) UpdateSettings(c *gin.Context) {
	...
	// 部分 PUT 的"只写客户端真正发来的键"由上游 omittedSettingKeys 承担：它按反射从
	// UpdateSettingsRequest 的值类型字段建表，未出现在请求体里的键进 omitted 并在写入前剔除，
	// 语义等价于 fork 原先的 presentKeys 白名单，且新增字段无需维护名单。
	//
	// OIDC 的 use_pkce / validate_id_token 是指针字段，被 buildSettingKeyByJSONName 有意跳过，
	// 因而永不进 omitted、始终随本次写入落库——fork 原先显式把这两个派生键加回白名单的处理
	// 在此机制下自动成立，无需另写。
	...
	settings := &service.SystemSettings{
		...
		SubscriptionsEnabled: func() bool {
			if req.SubscriptionsEnabled != nil {
				return *req.SubscriptionsEnabled
			}
			return previousSettings.SubscriptionsEnabled
		}(),
