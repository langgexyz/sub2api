...
func TestSettingService_PartialUpdate_OnlyWritesPresentKeys(t *testing.T) {
	...
	// 上游 omitted 语义：请求体里没出现的键进 omitted，落库前被剔除
	// （等价于 fork 原先的 presentKeys 白名单，见 setting_handler_update.go 的说明）。
	...
	err := svc.UpdateSettingsWithAuthSourceDefaultsOmitting(context.Background(), settings, authDefaults, omitted)
	require.NoError(t, err)
...
// behaviour for callers that pass no omitted set (and for the UpdateSettingsWithAuthSourceDefaults
...
func TestSettingService_PartialUpdate_NilMaskWritesAll(t *testing.T) {
	...
	err := svc.UpdateSettingsWithAuthSourceDefaultsOmitting(context.Background(),
