...
type Group struct {
	...
	// 订阅型 group 的固定槽位数 N：绑定号的容量按 N 等分给各订阅，默认 1（单订阅独占）
	SubscriptionSlots int `json:"subscription_slots,omitempty"`
...
func (*Group) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case group.FieldID, group.FieldSubscriptionSlots, group.FieldDefaultValidityDays, group.FieldFallbackGroupID, group.FieldFallbackGroupIDOnInvalidRequest, group.FieldSortOrder, group.FieldRpmLimit:
	...
	for i := range columns {
		...
		case group.FieldSubscriptionSlots:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for field subscription_slots", values[i])
			} else if value.Valid {
				_m.SubscriptionSlots = int(value.Int64)
			}
...
func (_m *Group) String() string {
	...
	builder.WriteString(", ")
	builder.WriteString("subscription_slots=")
	builder.WriteString(fmt.Sprintf("%v", _m.SubscriptionSlots))
	builder.WriteString(", ")
